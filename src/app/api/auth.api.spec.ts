...
import { mockGithubLoginResponse, mockLogin401Error, mockUserInfoFromAccessToken, TEST_ACCESS_TOKEN, TEST_GITHUB_CODE } from "./mock/auth.mock";
...
describe("AuthApi", () => {
    ...
    async function loginViaAccessToken(): Promise<void> {
        service.fetchUserInfoFromAccessToken(TEST_ACCESS_TOKEN).subscribe(
            () => async () => {
                ...
                await expect(service.currentUser).toBe(mockUserInfoFromAccessToken.auth.accountInfo);
            }
        );
        const op = controller.expectOne(FetchAccountInfoDocument);
        await expect(op.operation.variables.accessToken).toEqual(TEST_ACCESS_TOKEN);
        op.flush({
    ...
    async function loginFullyViaGithub(): Promise<void> {
        const localStorageSetItemSpy = spyOn(localStorage, 'setItem').and.stub();
        service.fetchUserInfoAndTokenFromGithubCallackCode(TEST_GITHUB_CODE).subscribe(
            () => {
                expect(service.isAuthenticated);
                void expect(service.currentUser).toBe(mockGithubLoginResponse.auth.githubLogin.accountInfo);
    ...
    async function checkUserIsLogged(user: AccountDetailsFragment): Promise<void> {
        ...
        async () => {
            ...
            service.onUserChanges.subscribe(
                (user: MaybeNull<AccountDetailsFragment>) => {
                    ...
                    user ? void checkUserIsLogged(user) : fail("User must not be null");
                }
            );
    ...
    it("should check full login GraphQL failure", fakeAsync(
        async () => {
            const handleAuthenticationErrorSpy = getSpy(service, 'handleAuthenticationError').and.callThrough();
            service.fetchUserInfoAndTokenFromGithubCallackCode(TEST_GITHUB_CODE).subscribe(() => { /* just need subscribe */ });
            ...
            expect(handleAuthenticationErrorSpy).toHaveBeenCalledWith([mockLogin401Error]);
        }
    ));
    ...
    it("should check login via access token GraphQL failure", fakeAsync(
        async () => {
            const handleAuthenticationErrorSpy = getSpy(service, 'handleAuthenticationError').and.callThrough();
            service.fetchUserInfoFromAccessToken(TEST_ACCESS_TOKEN).subscribe(() => { /* just need subscribe */ });
